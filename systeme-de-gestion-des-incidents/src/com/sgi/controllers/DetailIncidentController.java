				...
				try {
					...
					int confirmation = Utilitaire.showConfirmationMessage(
							"Assignation de l'incident\n"+
							 ...
							 "\t------------------------- \n"+
							 "Assigner à : "+user.getNom());
					if (confirmation == 0 )
					{
						
						incident.setStatut(Statut.ASSIGNED);						
						Service.updateIncident(incident,idDeveloppeur);
						Utilitaire.displayNotification("Incident assigné à < "+user.getNom()+" "+user.getPrenom()+" > avec succès !");
		...
		uiDetailIncident.addCloturerListener (new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					{
						Utilitaire.displayNotification("L'incident est déja cloturé");
					}else
					{
						...
						Utilitaire.displayNotification("Incident cloturé avec succès !");
						uiDetailIncident.getField(incident);
